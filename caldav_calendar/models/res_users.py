...
class ResUsers(models.Model):
    ...
    caldav_discovery_url = fields.Char(
    ...
    caldav_password = fields.Char(string='CalDAV Password', help='An app-specific password is strongly recommended.')
    caldav_sync_status = fields.Selection([
        ('not_configured', 'Not Configured'),
        ('active', 'Active'),
        ('paused', 'Paused'),
        ('error', 'Error'),
    ], string='CalDAV Sync Status', default='not_configured', copy=False)
    caldav_sync_token = fields.Char(copy=False, help='RFC 6578 sync-collection token.')
    caldav_sync_ctag = fields.Char(copy=False, help='Fallback change tag when sync-collection is unsupported.')
    caldav_last_sync = fields.Datetime(string='Last CalDAV Sync', copy=False)
    caldav_last_sync_error = fields.Text(string='Last CalDAV Sync Error', copy=False)

    @property
    def SELF_WRITEABLE_FIELDS(self):
        return super().SELF_WRITEABLE_FIELDS + [
            'caldav_discovery_url', 'caldav_url', 'caldav_username', 'caldav_password',
        ]
    ...
    def SELF_READABLE_FIELDS(self):
        return super().SELF_READABLE_FIELDS + [
            'caldav_discovery_url', 'caldav_url', 'caldav_username', 'caldav_password',
            'caldav_sync_status', 'caldav_last_sync', 'caldav_last_sync_error',
        ]
    ...
    def _caldav_get_client(self, url=None):
        self.ensure_one()
        target_url = url or self.caldav_url
        if not (target_url and self.caldav_username and self.caldav_password):
            raise UserError(self.env._('Please fill in the CalDAV URL, username and password first.'))
        return CalDAVClient(target_url, self.caldav_username, self.caldav_password)
    ...
    def action_caldav_test_connection(self):
        ...
        if self.caldav_sync_status == 'not_configured':
            self.caldav_sync_status = 'active'
        return {
    ...
    def action_caldav_discover_calendars(self):
        self.ensure_one()
        base_url = self.caldav_discovery_url or self.caldav_url
        if not (base_url and self.caldav_username and self.caldav_password):
    ...
    def action_caldav_sync_now(self):
        self.ensure_one()
        self._caldav_sync()
        if self.caldav_sync_status == 'error':
            error = self.caldav_last_sync_error or self.env._('unknown error')
    ...
    def _cron_caldav_sync_all(self):
        ...
        for user in users:
            try:
                ...
                _logger.exception('CalDAV: sync failed for user %s', user.login)
                user.write({
                    'caldav_sync_status': 'error',
                    'caldav_last_sync_error': traceback.format_exc(),
                })
    ...
    def _caldav_sync(self):
        self.ensure_one()
        user = self.sudo()
        client = user._caldav_get_client()
        try:
            user._caldav_push(client)
            user._caldav_pull(client)
        except CalDAVError as exc:
            user.write({'caldav_sync_status': 'error', 'caldav_last_sync_error': str(exc)})
            raise
        user.write({
            'caldav_last_sync': fields.Datetime.now(),
            'caldav_sync_status': 'active',
            'caldav_last_sync_error': False,
        })
    ...
    def _caldav_push(self, client):
        ...
        for pending in PendingDelete.search([('user_id', '=', self.id)]):
            try:
        ...
        dirty = Event.search([('need_caldav_sync', '=', True), ('user_id', '=', self.id)])
    ...
    def _caldav_pull(self, client):
        ...
        if deleted_hrefs:
            stale = Event.search([('caldav_href', 'in', deleted_hrefs), ('caldav_sync_user_id', '=', self.id)])
        ...
        self.write({'caldav_sync_token': new_token or self.caldav_sync_token})
    ...
    def _caldav_full_resync(self, client):
        ...
        local = self.env['calendar.event'].sudo().search([
            ('caldav_sync_user_id', '=', self.id), ('caldav_href', '!=', False),
        ])
        ...
        try:
            self.caldav_sync_ctag = client.get_ctag()
        except CalDAVError:
            pass
    ...
    def _caldav_apply_remote_event(self, item):
        ...
        existing = Event.search([
            ('caldav_href', '=', item['href']), ('caldav_sync_user_id', '=', self.id),
        ], limit=1)
        if not existing and master_info['vals'].get('caldav_uid'):
            existing = Event.search([
                ('caldav_uid', '=', master_info['vals']['caldav_uid']), ('caldav_sync_user_id', '=', self.id),
            ], limit=1)
    ...
    def _caldav_apply_overrides_and_exdates(self, master_event, overrides, exdates):
        ...
        for override in overrides:
            ...
            if occurrence:
                ...
                vals.update({'recurrence_id': recurrence.id, 'recurrency': False, 'user_id': self.id})
