...
| Delivered cleanly (2xx, no error, no Sidekiq retry/dead job) but the post never shows on Mastodon | Check the sending actor is genuinely **followed** by a *confirmed* local account there (not a pending follow request) - Mastodon's `Create` handler silently drops public posts from an actor with no local relationship, no error logged. If that checks out, also confirm this isn't a re-publish reusing an already-tombstoned object URI (fixed in the engine - a `Create` after a `Delete` always mints a new URI now) |
