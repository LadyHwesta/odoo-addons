...
# 19.0 port: base `membership` renamed `member_lines` -> `member_line_ids` and
# `associate_member` -> `associate_member_id`; `fields.first()` was removed
# (a plain `[:1]` recordset slice does the same thing).
