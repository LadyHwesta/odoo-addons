...
# 19.0 port: rebased onto `membership`'s own TestMembershipCommon
# (AccountTestInvoicingCommon) so a chart of accounts and a sale journal
# exist - the old SavepointCaseWithUserDemo base only passed on CI that
# also loaded accounting demo data. `account_invoice_line` ->
# `account_invoice_line_id` (renamed in `membership` 19.0).
